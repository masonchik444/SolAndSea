...
- [x] Данные владельца в подвале: Yaroslav Tyutyunikov · Autónomo ·
      ...
      03011 Alicante
- [x] Презентация для отелей: `presentacion.html`, печатается в PDF на
      трёх языках (см. раздел ниже)
...
- [x] Фотографии сжаты: 2,3 МБ → 1,2 МБ, первый кадр галереи 407 → 194 КБ.
      Требования к новым файлам — в `docs/PHOTOS.md`. Дальше вес можно
      снизить ещё примерно на треть переводом в WebP, но тогда картинки
      придётся подставлять через `image-set()` с запасным JPEG — сделаем,
      если скорость станет узким местом.

## Презентация для отеля (PDF)

Одностраничник на A4 открывается по адресу
`doremisolandsea.com/presentacion.html`. Порядок: выбрать язык
(ES / EN / RU) → **«Сохранить в PDF»** → в окне печати «Сохранить как
PDF», формат A4, поля «нет», масштаб 100%. Получается один лист, который
можно отправить в WhatsApp или почтой после звонка.

На листе: что мы делаем, три шага модели, таблица программ с ценами,
что получает отель (комиссия 15–30%, пример расчёта), три шага «как
начать», контакты, реквизиты и QR на сайт. Всё влезает в одну страницу
на каждом из трёх языков — при правке текстов это стоит проверять
(лист не должен уходить на вторую страницу).

Страница закрыта от индексации (`noindex`) и не входит в `sitemap.xml`:
это рабочий инструмент, а не раздел сайта.

## Таблички с QR для отелей

Макет открывается по адресу `doremisolandsea.com/qr-cartel.html` — оттуда
же печатается (A5, поля «нет», масштаб 100%). QR ведёт на `guest.html`,
страницу для гостя с программами и ценами.

Печатать лучше на плотной бумаге 250–300 г или заказать в типографии;
для стойки удобен формат «домиком» — тогда печатайте две таблички на A4.
